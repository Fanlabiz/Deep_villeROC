...
def build_excel(df, ville):
    ...
    # Marque les blocs non achetés (LOCKED_BLOCK) case par case en gris foncé
    # On colorie chaque case individuellement AVANT les bâtiments pour éviter les conflits
    locked_fill = PatternFill('solid', start_color='B0B0B0')
    locked_font = Font(name='Arial', size=7, bold=False, color='666666')
    locked_brd  = Border(
    ...
    for _, lb in locked_df.iterrows():
        ...
        # Colorie les 4x4 cases individuellement (pas de merge)
        for dr in range(4):
            for dc in range(4):
                er = lr1 + dr; ec = lc1 + dc
                ...
                if not isinstance(cell, _MC2):
                    cell.value     = 'x' if dr == 1 and dc == 1 else ''
                    cell.font      = locked_font
                    cell.fill      = locked_fill
                    cell.border    = locked_brd
