...
# Seuils hardcodés pour les bâtiments DynamicAge (f11 absent du CSV)
_DYNAMIC_THRESHOLDS = {
    'DynamicAge_Farm_Rural': {25: 1470, 50: 2940, 100: 5880},
}

# CityHall : culture dépend du relic power (valeur fixée)
_CITYHALL_CULTURE = 830
_CITYHALL_RANGE   = 4
...
_CULTURE_FORMULAS = {
    # Formules dynamiques (niveau + C) * D depuis DynamicFloatValueDefinitionDTO
    # Ces formules sont utilisées uniquement quand Culture=0 dans le CSV
    'Aqueduct': {
    ...
    # DynamicAge_CultureSite : formule (niveau+18)*D — vérifiée : niveau=14 → 1440 → D=45
    'DynamicAge_CultureSite': {
        'StoneAge':lambda l:(l+18)*10,'BronzeAge':lambda l:(l+18)*15,
        'MinoanEra':lambda l:(l+18)*20,'ClassicGreece':lambda l:(l+18)*25,
        'EarlyRome':lambda l:(l+18)*28,'RomanEmpire':lambda l:(l+18)*32,
        'ByzantineEra':lambda l:(l+18)*35,'AgeOfTheFranks':lambda l:(l+18)*37,
        'FeudalAge':lambda l:(l+18)*39,'IberianEra':lambda l:(l+18)*41,
        'KingdomOfSicily':lambda l:(l+18)*42,'HighMiddleAges':lambda l:(l+18)*43,
        'EarlyGothicEra':lambda l:(l+18)*44,'LateGothicEra':lambda l:(l+18)*45,
    },
}

# Données de référence pour les Evolving : liste de (level, culture, rayonnement)
# On interpole linéairement entre les points connus pour tout niveau intermédiaire
_EVOLVING_POINTS = {
    'MotherTree':          [(1,157,1),(10,300,1),(23,386,1),(60,1100,4)],
    'CelticArch':          [(1,288,1),(10,450,1),(13,468,1),(60,1350,4)],
    'ElysianField':        [(1,40,1),(10,289,1),(16,330,1),(60,704,4)],
    'GriotCourt':          [(1,192,1),(7,288,1),(10,336,1),(60,1136,4)],
    'CryptOfTheCount':     [(1,60,1),(10,193,1),(24,404,2),(60,713,4)],
    'FountainOfYouth':     [(1,120,1),(10,420,1),(25,888,2),(60,1370,5)],
    'PirateFortress':      [(1,120,1),(10,458,2),(21,716,4),(60,1490,5)],
    'GreatGarden':         [(1,55,1),(10,219,1),(19,301,2),(60,664,4)],
    'AztecMainTemple':     [(1,208,1),(10,352,1),(26,608,2),(60,1152,4)],
    'Madrasa':             [(1,0,0),(10,96,1),(21,182,2),(60,374,3)],
    'WinterMarket':        [(1,94,1),(10,396,2),(12,396,2),(60,1450,4)],
    'RoyalTemple':         [(1,240,1),(10,425,1),(23,646,2),(60,1275,4)],
    'DrumTower':           [(1,127,1),(10,506,1),(26,830,3),(60,1800,7)],
    'DovecoteTower':       [(1,122,1),(10,467,1),(20,738,2),(60,1530,5)],
    'Woodhenge':           [(1,250,2)],
    'Astronomer':          [(1,125,3)],
    'MinoanWatchtowerV2':  [(1,1350,3)],
}

def _interp_evolving(points, level):
    """Interpole culture et rayonnement pour un niveau donné."""
    pts = sorted(points, key=lambda p: p[0])
    if level <= pts[0][0]: return pts[0][1], pts[0][2]
    if level >= pts[-1][0]: return pts[-1][1], pts[-1][2]
    for i in range(len(pts)-1):
        n1, c1, r1 = pts[i]
        n2, c2, r2 = pts[i+1]
        if n1 <= level <= n2:
            t = (level - n1) / (n2 - n1)
            cult = int(round(c1 + t * (c2 - c1)))
            ray = r1 if level < n2 else r2
            # Rayonnement suit les paliers (pas d'interpolation)
            ray = r1
...
_RANGE_FORMULAS = {
    ...
    'DynamicAge_CultureSite': lambda l: 1 if l < 10 else (2 if l < 30 else 3),
}

def compute_evolving_culture(nom_complet, niveau, ere_index):
    """Retourne (culture, rayonnement) pour un Evolving/DynamicAge."""
    ere = _ERES_PAR_INDEX.get(ere_index, 'LateGothicEra')
    # 0. CityHall : valeur fixe (dépend du relic power)
    if 'CityHall' in nom_complet:
        return _CITYHALL_CULTURE, _CITYHALL_RANGE
    # 1. Formule dynamique exacte (Aqueduct, DynamicAge_CultureSite)
    for key, formulas in _CULTURE_FORMULAS.items():
        if key in nom_complet:
            fn = formulas.get(ere, formulas.get('LateGothicEra'))
            if fn:
                ray_fn = _RANGE_FORMULAS.get(key)
                ray = ray_fn(niveau) if ray_fn else 1
                return int(round(fn(niveau))), ray
    # 2. Interpolation sur les points de référence
    for key, points in _EVOLVING_POINTS.items():
        if key in nom_complet:
            return _interp_evolving(points, niveau)
    return 0, 0
...
def compute_evolving_range(nom_complet, niveau):
    for key, fn in _RANGE_FORMULAS.items():
        if key in nom_complet:
            return fn(niveau)
    for key, (cult, ray) in _EVOLVING_REF.items():
        if key in nom_complet:
            return ray
    return 0
...
def _get_threshold(full_name, pct):
    ...
    for key, vals in _DYNAMIC_THRESHOLDS.items():
        if key in full_name:
            return vals.get(pct, '')
...
CAT_COLORS = {
    'Culturel':   'FFB347',   # orange
    'Caserne':    'A8D5A2',   # vert
    'Production': 'A8D5A2',   # vert
    'Habitation': 'A8D5A2',   # vert
    'Neutre':     'DCDCDC',   # gris
}
...
def get_color(name, range_val):
    """Orange si produit de la culture (range_val = rayonnement calculé final),
    vert si producteur/habitation/caserne, gris sinon."""
    if range_val > 0 and is_real_culture(name, range_val):
        return 'FFB347'   # orange
    cat = get_cat(name)
    if cat in ('Habitation', 'Production', 'Caserne'):
        return 'A8D5A2'   # vert
    return 'DCDCDC'       # gris
...
def build_excel(df, ville):
    ...
    if rows_df.empty:
        return None
    # Sépare les bâtiments des blocs d'expansion
    # PURCHASED_BLOCK = blocs achetés (présents dans f6 du CityDTO)
    # locked = blocs de la grille ABSENTS du CSV → calculés ci-dessous
    purchased_df = rows_df[rows_df['Nom_complet'] == 'PURCHASED_BLOCK'].copy()
    rows_df      = rows_df[~rows_df['Nom_complet'].isin(['PURCHASED_BLOCK','LOCKED_BLOCK'])].copy()

    # Grille complète des blocs 4x4 : col 11..47 step 4, row 15..47 step 4
    purchased_set = set(zip(
        purchased_df['Colonne'].astype(int),
        purchased_df['Ligne'].astype(int)
    ))
    all_blocks = [(c, r) for c in range(11, 48, 4) for r in range(15, 48, 4)]
    locked_coords = [(c, r) for c, r in all_blocks if (c, r) not in purchased_set]

    # Crée un DataFrame locked_df synthétique
    import io as _io
    locked_rows = [{'Ville': ville, 'Nom_complet': 'LOCKED_BLOCK',
                    'Largeur': 4, 'Hauteur': 4, 'Culture': 0, 'Rayonnement': 0,
                    'Niveau': 0, 'Seuil25': 0, 'Seuil50': 0, 'Seuil100': 0,
                    'Rotation': 0, 'Colonne': c, 'Ligne': r}
                   for c, r in locked_coords]
    locked_df = pd.DataFrame(locked_rows) if locked_rows else pd.DataFrame()
    ...
    # Marque les cases non utilisables avec 'X' sur fond blanc
    # Sources : (1) blocs locked du CSV, (2) bords du terrain
    x_font = Font(name='Arial', size=8, bold=False, color='AAAAAA')
    from openpyxl.cell.cell import MergedCell as _MC2

    # Collecte les cases occupées par des bâtiments
    bld_cells = set()
    for rng in ws_t.merged_cells.ranges:
        for er in range(rng.min_row, rng.max_row+1):
            for ec in range(rng.min_col, rng.max_col+1):
                bld_cells.add((er, ec))

    # (1) Blocs locked depuis le CSV : X sur chaque case non occupée
    locked_cells_set = set()
    for _, lb in locked_df.iterrows():
        c0, r0 = int(lb['Colonne']), int(lb['Ligne'])
        lr1 = n_cols - c0 - 4 + 1
        lc1 = n_rows - r0 - 4 + 2
        lr1 = max(2, lr1); lc1 = max(2, lc1)
        for dr in range(4):
            for dc in range(4):
                er = lr1 + dr; ec = lc1 + dc
                if er >= 2 and ec >= 2:
                    locked_cells_set.add((er, ec))

    # (2) Bords du terrain : limites exactes
    # Gauche : col_jeu 7-10, row_jeu 15-51
    # Droit  : col_jeu 51,   row_jeu 15-51
    # Bas    : row_jeu 11-14, col_jeu 11-47
    # Haut   : row_jeu 51,   col_jeu 7-51
    for er in range(2, ws_t.max_row + 1):
        for ec in range(2, ws_t.max_column + 1):
            c_jeu = n_cols - er
            r_jeu = n_rows - ec + 1
            is_border = (
                (7  <= c_jeu <= 10 and 15 <= r_jeu <= 51) or
                (c_jeu == 51       and 15 <= r_jeu <= 51) or
                (11 <= r_jeu <= 14 and 11 <= c_jeu <= 47) or
                (r_jeu == 51       and  7 <= c_jeu <= 51)
            )
            if is_border:
                locked_cells_set.add((er, ec))

    # Applique les X
    for (er, ec) in locked_cells_set:
        if (er, ec) in bld_cells: continue
        if er < 2 or ec < 2: continue
        cell = ws_t.cell(er, ec)
        if isinstance(cell, _MC2): continue
        cell.value     = 'X'
        cell.font      = x_font
        cell.alignment = ctr
    ...
    for row_idx, (_, b) in enumerate(rows_df.iterrows(), 2):
        ...
        if is_real_culture(nom, ray_raw):
            # Le CSV contient déjà les valeurs calculées (nouveau userscript)
            # Fallback : calcul Streamlit si Culture=0 (ancien CSV)
            if cult_raw > 0:
                cult, ray = cult_raw, ray_raw
            else:
                cult, ray = compute_evolving_culture(nom, niveau_b, ere_idx)
        else:
            cult, ray = 0, 0
        # Type final : Culturel si le bâtiment produit vraiment de la culture
        cat = 'Culturel' if (cult > 0 and ray > 0) else cat_raw
